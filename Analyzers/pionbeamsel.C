...
void pionbeamsel::FillBeamPlots(TString beam_selec_str, double weight){
  ...
  if(evt.reco_beam_type == pandora_slice_pdg) beam_reco_as_trk = 0.5;
  else beam_reco_as_trk = -0.5;

  double beam_calo_size = -2.;
  if(!(evt.reco_beam_calo_wire->empty())) beam_calo_size = 0.5;
  else beam_calo_size = -0.5;
  ...
  if(Z_dir_sign < 0.) Z_dir_sign = -0.5;
  else Z_dir_sign = 0.5;
