...
std::vector<Daughter> AnalyzerCore::GetPions(const vector<Daughter>& in) {
    ...
    for (unsigned int i = 0; i < in.size(); i++) {
        ...
        if (this_in.PFP_trackScore() > cut_trackScore && this_in.PFP_emScore() < cut_emScore && this_chi2 > cut_chi2_proton && this_in.PFP_nHits() > cut_Nhit && this_in.Beam_Cos() < cut_cos_beam && this_in.Beam_Dist() < cut_beam_dist && this_in.allTrack_startZ() < cut_startZ) {
            out.push_back(this_in);
        }
    }
...
std::vector<Daughter> AnalyzerCore::GetStoppingPions(const vector<Daughter>& in, double chi2_pion_cut) {
    ...
    for (unsigned int i = 0; i < in.size(); i++) {
        ...
        if (this_in.PFP_trackScore() > cut_trackScore && this_in.PFP_emScore() < cut_emScore && this_chi2 > cut_chi2_proton && this_in.PFP_nHits() > cut_Nhit && this_in.Beam_Cos() < cut_cos_beam && this_in.Beam_Dist() < cut_beam_dist && this_in.allTrack_startZ() < cut_startZ && this_chi2_pion < chi2_pion_cut) {
            out.push_back(this_in);
        }
    }
    ...
    if (start_idx >= 0) {
        ...
        for (int i_true_hit = start_idx + 1; i_true_hit < (*evt.true_beam_traj_Z).size(); i_true_hit++) {
            if (i_true_hit == start_idx + 1) {
                true_trklen = sqrt(pow((*evt.true_beam_traj_X).at(i_true_hit) - (*evt.true_beam_traj_X).at(i_true_hit - 1), 2) + pow((*evt.true_beam_traj_Y).at(i_true_hit) - (*evt.true_beam_traj_Y).at(i_true_hit - 1), 2) + pow((*evt.true_beam_traj_Z).at(i_true_hit) - (*evt.true_beam_traj_Z).at(i_true_hit - 1), 2)) * (*evt.true_beam_traj_Z).at(i_true_hit) / ((*evt.true_beam_traj_Z).at(i_true_hit) - (*evt.true_beam_traj_Z).at(i_true_hit - 1));
            } else {
                true_trklen += sqrt(pow((*evt.true_beam_traj_X).at(i_true_hit) - (*evt.true_beam_traj_X).at(i_true_hit - 1), 2) + pow((*evt.true_beam_traj_Y).at(i_true_hit) - (*evt.true_beam_traj_Y).at(i_true_hit - 1), 2) + pow((*evt.true_beam_traj_Z).at(i_true_hit) - (*evt.true_beam_traj_Z).at(i_true_hit - 1), 2));
            }
...
double AnalyzerCore::Get_true_beamlen() {
    ...
    if (start_idx >= 0) {
        ...
        for (int i_true_hit = start_idx + 1; i_true_hit < (*evt.true_beam_traj_Z).size(); i_true_hit++) {
            if (i_true_hit == start_idx + 1) {
                beamlen_true = sqrt(pow((*evt.true_beam_traj_X).at(i_true_hit) - (*evt.true_beam_traj_X).at(i_true_hit - 1), 2) + pow((*evt.true_beam_traj_Y).at(i_true_hit) - (*evt.true_beam_traj_Y).at(i_true_hit - 1), 2) + pow((*evt.true_beam_traj_Z).at(i_true_hit) - (*evt.true_beam_traj_Z).at(i_true_hit - 1), 2)) * (*evt.true_beam_traj_Z).at(i_true_hit) / ((*evt.true_beam_traj_Z).at(i_true_hit) - (*evt.true_beam_traj_Z).at(i_true_hit - 1));
            } else {
                beamlen_true += sqrt(pow((*evt.true_beam_traj_X).at(i_true_hit) - (*evt.true_beam_traj_X).at(i_true_hit - 1), 2) + pow((*evt.true_beam_traj_Y).at(i_true_hit) - (*evt.true_beam_traj_Y).at(i_true_hit - 1), 2) + pow((*evt.true_beam_traj_Z).at(i_true_hit) - (*evt.true_beam_traj_Z).at(i_true_hit - 1), 2));
            }
        }
    }
...
bool AnalyzerCore::Pass_Beam_PID(int PID) {
    ...
    if (evt.MC) {
        ...
        if (evt.beam_inst_nMomenta != 1 || evt.beam_inst_nTracks != 1) return false;  // single track
        for (size_t i = 0; i < PDGID_candiate_vec.size(); ++i) {
